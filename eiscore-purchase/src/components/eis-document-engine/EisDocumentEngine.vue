...
const isEditableField = (item) => {
  ...
  if (col && typeof col.statusEditable === 'function' && !col.statusEditable(props.modelValue || {})) return false
  if (col?.statusEditable === false) return false
  if (!col) {
...
const isTableEditable = (section, col) => {
  ...
  if (meta && typeof meta.statusEditable === 'function' && !meta.statusEditable(props.modelValue || {})) return false
  if (meta?.statusEditable === false) return false
  if (!meta) return true
