        ...
        <el-tabs v-model="internalTab" class="config-tabs">
          <el-tab-pane label="基础统计" name="basic">
            ...
            <div class="cell-label-block">
              ...
              <el-input v-model="internalCellLabel" placeholder="比如：订单金额" clearable />
              <span class="cell-label-tip">显示效果：订单金额：123</span>
            </div>
          </el-tab-pane>
          ...
          <el-tab-pane label="高级公式" name="formula">
            <p class="dialog-tip">
              <b>列间运算公式：</b><br>
              <span style="font-size: 12px; color: #909399;">例如: <code>{订单金额} * 0.18</code></span>
            </p>
...
const buildFormulaPrompt = () => {
  ...
  return [
    ...
    `目标列：${colName}`,
    '要求：只输出公式，不要解释。',
    '必须放在 ```formula``` 代码块中，内容示例：{订单金额}*0.18。',
    `可用字段：${variables || '无'}。`
  ].join('\n')
}
