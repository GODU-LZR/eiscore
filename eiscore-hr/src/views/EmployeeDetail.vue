    ...
    <div class="page-header" data-guide="detail-header">
      ...
      <div class="header-actions" data-guide="detail-actions">
        ...
        <el-button
          ...
          data-sop-desc="管理员工档案或人事单据模板，可新增、预览、改名或删除模板。"
          data-sop-steps="先确认当前人事应用类型|打开模板库|预览或选择模板|只删除确认废弃的模板|回到单据页重新选择模板并复核显示效果"
        ...
        <el-button
          ...
          data-sop-desc="用 AI 辅助生成当前人事单据模板或字段建议，生成后必须人工复核。"
          data-sop-steps="先确认当前员工或人事业务类型|点击 AI生成表单|检查生成字段是否符合档案、组织、考勤或调岗场景|删除不需要的字段并补齐关键项|保存前由人事负责人复核"
          data-sop-risk="AI 只能辅助生成模板，不能代替个人信息准确性、权限控制和人事审批留痕。"
        ...
        <el-button
          ...
          data-sop-title="打印单据"
          data-sop-desc="按当前模板打印或导出当前人事单据。"
          data-sop-steps="先确认模板、员工姓名、部门、岗位和状态|点击打印单据|检查打印预览中的页边距、字段和签字栏|打印或导出 PDF 后按制度留档"
          @click="printDoc"
        >打印单据</el-button>
        <el-button
          type="success"
          ...
          data-sop-desc="保存当前人事单据正文、扩展字段和附件，保存前必须复核个人信息、部门、岗位和状态。"
          data-sop-steps="先复核员工姓名、工号、部门、岗位、状态和负责人|检查必填项和风险提示|确认附件已上传且内容正确|点击保存修改|保存后回到表格搜索该记录复核状态"
          data-sop-risk="保存后请回到人事表格搜索该记录，确认员工档案、组织关系和权限影响都正确。"
          @click="saveDoc"
        >保存修改</el-button>
      </div>
    </div>
...
const syncAiContext = () => {
  ...
  pushAiContext(buildDocumentAgentContext({
    ...
    apiUrl: detailConfig.value.apiUrl || '/archives',
    writeUrl: detailConfig.value.writeUrl || detailConfig.value.apiUrl || '/archives',
...
const openAiFormAssistant = () => {
  ...
  const prompt = buildDocumentFormPrompt({
    title: detailConfig.value.name || '员工单据',
    columns,
    rowData: formModel.value || formData.value
  })
  pushAiCommand({
