...
测试目标：补齐浏览器级自动化验收，验证页面真实渲染、登录态注入、主站与关键微应用深链无白屏，并覆盖普通用户日常 UI 点击链路。
...
| 根命令 | `package.json` | 新增 `test:e2e`、`test:e2e:remote`、`test:e2e:clicks`、`test:e2e:clicks:remote`、`e2e:install`、`e2e:install:with-deps`。 |
...
1. 浏览器级 E2E 第一版已完成，远端 `nanpai.eissys.top` 通过 9/9。
2. 当前 E2E 已能捕获登录页白屏、主站 shell 白屏、关键微应用深链加载失败等问题。
3. 新增 UI 点击巡检已覆盖普通用户日常点击入口，可捕获登录按钮、侧边导航、应用卡片、网格工具栏、应用中心弹窗点击异常。
4. 本轮不把 E2E 纳入默认 `test:ci`，避免依赖外部远端环境和浏览器系统库影响普通 PR 构建。
5. 下一阶段建议扩展为带测试数据隔离的真实表单提交 E2E，例如创建后清理 HR 档案、仓库、流程审批记录。
