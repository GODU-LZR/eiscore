...
export function useGridHistory(props, gridApi, gridData, formulaHooks) {
            ...
            if (writeMode === 'patch') {
              await Promise.all(entries.map(async (entry) => {
                ...
                await request({
                  url: patchUrl,
                  method: 'patch',
                  headers: { 'Accept-Profile': props.acceptProfile || 'public', 'Content-Profile': props.contentProfile || 'public', 'Prefer': 'return=representation' },
                  data: patchPayload
                })
              }))
              ...
              await request({
                  ...
                  headers: { 'Accept-Profile': props.acceptProfile || 'public', 'Content-Profile': props.contentProfile || 'public', 'Prefer': 'resolution=merge-duplicates,return=representation' },
                  data: apiPayload
              })
  ...
  const sanitizeValue = (field, value) => {
    ...
    const textFields = ['name', 'code', 'username', 'email', 'phone', 'address', 'status', 'customer_no', 'order_no', 'payment_no', 'customer_name', 'product_name', 'contact_name', 'contact_phone', 'region', 'owner_name', 'handler_name']
