...
const getRealtimeTable = () => {
  const apiUrl = app.value?.apiUrl || ''
  const raw = apiUrl.startsWith('/') ? apiUrl.slice(1) : apiUrl
  return raw.split('?')[0]
}
...
const handleRealtimeEvent = (event) => {
  ...
  if (!payload) return
  const targetSchema = appProfile.value || 'public'
  const targetTable = getRealtimeTable()
  if (payload.schema === targetSchema && payload.table === targetTable) {
    scheduleGridReload()
  }
}
