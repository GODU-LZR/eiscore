...
import {
  MATERIAL_ATTENTION_LEVEL_OPTIONS,
  attentionLevelRank,
...
export function useGridCore(props, activeSummaryConfig, currentUser, isCellInSelection, gridApiRef, emit, workflowBindingRef) {
  ...
  const getCellStyle = (params) => {
    ...
    if (params.colDef.isAttentionColumn) return { ...base, cursor: params.colDef.editable === false ? 'default' : 'pointer' }
  ...
  const createColDef = (col, isDynamic) => {
    ...
    const configuredWidth = getStoredColumnWidth(colId, col.width)
  ...
  const gridColumns = computed(() => {
    ...
    const attentionWritable = props.includeProperties !== false && props.canEdit !== false
    const attentionCol = {
      headerName: '关注',
      ...
      cellEditorPopup: true,
      cellEditorPopupPosition: 'under',
      options: MATERIAL_ATTENTION_LEVEL_OPTIONS,
      allowClear: false,
      valueGetter: params => {
