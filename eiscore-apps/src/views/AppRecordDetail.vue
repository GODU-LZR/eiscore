...
const templateScope = computed(() => ({
  app: 'app_center',
  key: `app_${appId.value || 'data'}`,
  appId: appId.value || '',
  configKey: `app_${appId.value || 'data'}_form`,
  apiUrl: apiUrl.value || ''
}))
const defaultTemplateId = computed(() => `default_${templateScope.value.app}_${templateScope.value.key}`)
const defaultTemplateName = computed(() => `${appName.value || '数据应用'}默认表单`)
...
const isLegacyAppTemplate = (template) => {
  const docType = String(template?.schema?.docType || template?.docType || '').toLowerCase()
  const expected = `app_${appId.value || 'data'}_auto`.toLowerCase()
  return docType === expected
}
...
const isTemplateForCurrentScope = (template) => {
  ...
  if (scope && typeof scope === 'object') {
    ...
    if (scope.appId && scope.appId !== templateScope.value.appId) return false
    ...
    if (scope.app || scope.key || scope.appId || scope.configKey || scope.apiUrl) return true
  }
  return isLegacyAppTemplate(template)
}
...
const loadFormValues = async () => {
  ...
  try {
    let res = await fetchFormValues(rowId.value, detailFormValueKey.value)
    if ((!Array.isArray(res) || res.length === 0) && detailFormValueKey.value !== selectedTemplateId.value) {
      res = await fetchFormValues(rowId.value, selectedTemplateId.value)
    }
    if (res && res.length > 0) {
