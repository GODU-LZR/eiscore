...
draw.text(((W - (bb[2] - bb[0])) / 2, 30), title, font=F_TITLE, fill="black")

# system boundary
bx1, by1, bx2, by2 = 360, 150, 2040, 1180
...
draw.rectangle((980, 140, 1420, 200), fill="white")
draw.text(((W - (bb[2] - bb[0])) / 2, 152), bound_title, font=F_BOUND, fill="black")

# actors
actor(draw, 130, 380, "仓管员")
actor(draw, 130, 820, "生产主管")
actor(draw, 2270, 380, "销售文员")
actor(draw, 2270, 820, "系统管理员")

# use cases (inside boundary)
cases = {
    "领料": (560, 260, 420, 110, "生产领料"),
    "补料": (560, 460, 420, 110, "生产补料"),
    "入库": (560, 660, 420, 110, "生产入库"),
    "出库": (560, 860, 420, 110, "销售出库"),
    "台账": (1380, 260, 420, 110, "库存台账查询"),
    "批次": (1380, 460, 420, 110, "批次与保质期查看"),
    "预警": (1380, 660, 420, 110, "库存预警与状态核对"),
}
for v in cases.values():
    usecase(draw, *v)

# actor associations (plain UML associations)
assoc(draw, (165, 330), (560, 315))   # 仓管员-生产领料
assoc(draw, (165, 355), (560, 515))   # 仓管员-生产补料
assoc(draw, (165, 770), (560, 715))   # 生产主管-生产入库
assoc(draw, (165, 795), (560, 515))   # 生产主管-生产补料
assoc(draw, (2235, 330), (1800, 315)) # 销售文员-库存台账查询
assoc(draw, (2235, 355), (980, 915))  # 销售文员-销售出库
assoc(draw, (2235, 770), (1800, 515)) # 系统管理员-批次与保质期查看
assoc(draw, (2235, 795), (1800, 715)) # 系统管理员-库存预警与状态核对
assoc(draw, (2235, 820), (1800, 315)) # 系统管理员-库存台账查询

out = Path("/home/lzr/eiscore/docs/diagrams/图3-3_仓储与库存作业用例图_候选版3.png")
