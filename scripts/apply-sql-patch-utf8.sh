...
PATCH_FILE="sql/patch_fix_ontology_semantic_qmarks_20260618.sql"
...
garbled_column_semantics="$(docker exec "${DB_CONTAINER}" psql -U "${DB_USER}" -d "${DB_NAME}" -Atc "select count(*) from public.ontology_column_semantics where semantic_name like '%?%' or semantic_description like '%?%';" | tr -d '\r')"
...
echo "garbled_table_semantics=${garbled_semantics}"
echo "garbled_column_semantics=${garbled_column_semantics}"
...
if [[ "${garbled_semantics}" != "0" || "${garbled_column_semantics}" != "0" || "${garbled_relations}" != "0" ]]; then
